...
import tv.quaint.streamlinebase.utils.obj.NodeDocument;
        ...
        if (this.configuration.of.equals(DatabaseConfiguration.SupportedType.MONGO)) {
            ...
            try {
                if (collection.find(query).first() == null) {
                    if (isCreating) {
                        StreamlineBase.SERVER.getScheduler().schedule(StreamlineBase.INSTANCE, this::create, 5, TimeUnit.SECONDS);
                    } else {
                        create();
                    }
                }
