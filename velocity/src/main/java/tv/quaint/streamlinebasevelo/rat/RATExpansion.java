package tv.quaint.streamlinebasevelo.rat;
...
public abstract class RATExpansion {
    ...
    public RATExpansion removeCallForUser(SavablePlayer user, int remove) {
        if (! this.userCalls.containsKey(user.getUniqueId().toString())) return this.setCallForUser(user, remove);
        else {
            return this.setCallForUser(user, this.userCalls.get(user.getUniqueId().toString()) - remove);
        }
    }
    ...
    public RATExpansion(String identifier, String author, String version) {
        ...
        StreamlineBase.ratAPI.registerExpansion(this);
    }
